...
{{- if not .Values.auth.disabled -}}
  ...
  {{- else -}}
    {{- $hasProvider := or .Values.auth.github.clientId (or .Values.auth.google.clientId (or .Values.auth.gitlab.clientId (or .Values.auth.oidc.issuerUrl .Values.github.app.clientId))) -}}
    {{- if not $hasProvider -}}
      {{- fail "At least one OAuth provider must be configured when auth is enabled. Set auth.github.clientId, auth.google.clientId, auth.gitlab.clientId, auth.oidc.issuerUrl, or github.app.clientId (with corresponding clientSecret)." -}}
    {{- end -}}
  {{- end -}}
{{- end -}}
{{- end }}
