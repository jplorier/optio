...
export function ReviewDraftPanel({ taskId, taskState }: ReviewDraftPanelProps) {
  ...
  // Auto-refresh while drafting or waiting on CI
  useEffect(() => {
    if (draft?.state !== "drafting" && draft?.state !== "waiting_ci") return;
    const interval = setInterval(fetchDraft, 5000);
    return () => clearInterval(interval);
  }, [draft?.state, fetchDraft]);

  // Chat: fetch on draft load, poll while a turn is in flight.
  const fetchChat = useCallback(async () => {
    try {
      const res = await api.listReviewChat(taskId);
  ...
  useEffect(() => {
    if (draft && ["ready", "stale", "submitted"].includes(draft.state)) {
      fetchChat();
    }
  }, [draft, fetchChat]);
  ...
  useEffect(() => {
    ...
    return () => clearInterval(interval);
  }, [chatSending, fetchChat, fetchDraft]);
  ...
  const handleSendChat = async () => {
    ...
    setChatSending(true);
    try {
      await api.postReviewChat(taskId, message);
      toast.success("Sent to agent");
      // Wait for the agent's reply — polling in effect above will refresh.
      // Stop the "sending" indicator once a new assistant message arrives.
      const start = chatMessages.length;
      let attempts = 0;
      const waitInterval = setInterval(async () => {
        attempts += 1;
        const res = await api.listReviewChat(taskId).catch(() => null);
        if (res && res.messages.some((m, i) => i > start && m.role === "assistant")) {
          setChatMessages(res.messages);
  ...
  // ── Waiting for CI ────────────────────────────────────────────────────────
  if (draft.state === "waiting_ci") {
    return (
      <div className="rounded-lg border border-border bg-bg-card p-4 mb-4">
        <div className="flex items-center gap-2 text-text-muted text-sm">
          <Clock className="w-4 h-4" />
          <span className="font-medium">Waiting for CI to finish before reviewing...</span>
        </div>
        <p className="text-xs text-text-muted mt-1">
          Optio is watching this PR and will auto-review once CI completes.
        </p>
      </div>
    );
  }
    ...
    <div className="rounded-lg border border-border bg-bg-card p-4 mb-4 space-y-4">
      ...
      {draft.origin === "auto" && (
        <div className="flex items-center gap-2 text-xs text-text-muted">
          ...
          <span>
            Automatically generated by Optio
            {draft.autoSubmitted ? " — already posted to the PR" : ""}
          </span>
        </div>
      )}
      ...
      <div className="pt-3 border-t border-border">
        ...
        <p className="text-[11px] text-text-muted mb-2">
          Ask follow-up questions. The agent may update the draft above if your message calls for
          it.
        </p>
